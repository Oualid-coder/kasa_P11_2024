...
function Carrousel({ slides }) {
  ...
  const [current, setCurrent] = useState(0); //on définie l'index du premier slide à 0
  const length = slides.length; //longueur du tableau de slides

  /**Fonction pour l'image precedente */
  const nextImage = () => {
    setCurrent(current === length - 1 ? 0 : current + 1); // on passe au premier slide une fois arriver au dernier
  };
  /**Function pour l'image suivante */
  const prevImage = () => {
    setCurrent(current === 0 ? length - 1 : current - 1); // on passe au dernier slide une fois arriver au premier
  };
